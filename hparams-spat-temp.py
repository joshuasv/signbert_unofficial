...
def objective(trial: optuna.trial.Trial, dpath, name, device, acc_grad_batches) -> float:
    ...
    m = 5 # fixed
    K = 8 # fixed
    max_disturbance=trial.suggest_float('max_disturbance', 2, 50.0, log=True) # Maximum disturbance to add to joints when joint masking
    batch_size = 16 // acc_grad_batches # fixed
    ...
    num_hid = 128 # fixed
    num_heads = 4 # fixed
    tformer_n_layers = 4 # fixed
    tformer_dropout = 0.25 # fixed
    eps = 0.5 # fixed
    hand_cluster = True # fixed
    n_pca_components = trial.suggest_categorical('n_pca_components', [11, 15, 25, 35])
    gesture_extractor_cls = 'signbert.model.GestureExtractorSpatTemp.GestureExtractor' # fixed
    ...
    trainer = pl.Trainer(
        deterministic="warn", # Because of padding not being deterministic
        ...
        callbacks=[
            ...
            # EarlyStopping(monitor="val_PCK_20", mode="max", patience=30, min_delta=1e-4), # 300 epochs patience (check_val_every_n_epoch * patience)
        ],
    )
